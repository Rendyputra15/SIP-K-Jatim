...
class _AdminUserManagementScreenState extends State<AdminUserManagementScreen> {
  ...
  void _showUserFormDialog({AppUser? userToEdit}) {
    ...
    final deptController = TextEditingController(text: userToEdit?.department ?? '');
    ...
    UserRole selectedRole = userToEdit?.role ?? (widget.currentUser.isSuperAdmin ? UserRole.admin : UserRole.user);
              ...
              child: Form(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    // Input Nama
                    _buildTextField(nameController, 'Nama Lengkap & Gelar', 'Contoh: Rendy Cahyono, S.Kom'),
                    const SizedBox(height: 12),

                    // Input NIP
                    _buildTextField(nipController, 'NIP Pegawai', 'Contoh: 199503152020121002'),
                    const SizedBox(height: 12),

                    // Input Bidang
                    _buildTextField(deptController, 'Bidang / Seksi / Bagian', 'Contoh: Bidang Linjamsos'),
                    const SizedBox(height: 12),

                    // Input Email
                    _buildTextField(emailController, 'Email Resmi Dinsos', 'nama@dinsos.jatimprov.go.id'),
                    const SizedBox(height: 12),

                    // Pemilihan Role
                    const Text('Peran Pengguna (Hak Akses)', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Color(0xFF1E293B))),
                    ...
                    DropdownButtonFormField<UserRole>(
                      ...
                      items: [
                        if (widget.currentUser.isSuperAdmin) ...[
                          const DropdownMenuItem(value: UserRole.admin, child: Text('Admin (Kasubag / Aset)', style: TextStyle(fontSize: 13))),
                        ],
                        const DropdownMenuItem(value: UserRole.user, child: Text('Pegawai (User Pemohon)', style: TextStyle(fontSize: 13))),
                      ],
                      onChanged: (val) {
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton(
                        ...
                        child: Text(isEdit ? 'Perbarui Data Akun' : 'Simpan Akun Baru', style: const TextStyle(fontWeight: FontWeight.bold)),
                      ),
                    ),
                  ],
                ),
  ...
  void _showDeleteConfirmDialog(AppUser user) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
        title: const Text('Hapus Akun?', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: Color(0xFFDC2626))),
        content: Text('Apakah Anda yakin ingin menghapus akun ${user.name} (${user.role.name.toUpperCase()}) dari sistem?'),
        actions: [
    ...
    return Column(
      ...
      children: [
        ...
        TextFormField(
          ...
          validator: (val) => val == null || val.isEmpty ? 'Kolom ini wajib diisi' : null,
        ),
      ],
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(
          widget.currentUser.isSuperAdmin ? 'Kelola Akun Admin & Pegawai' : 'Kelola Akun Pegawai (User)',
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        label: Text(
          widget.currentUser.isSuperAdmin ? 'Tambah Admin / User' : 'Tambah User Pegawai',
          ...
          : ListView.builder(
              ...
              itemBuilder: (ctx, i) {
                ...
                final bool canManage = widget.currentUser.isSuperAdmin
                    ...
                    : !isAdminRole && !isSuper; // Admin biasa hanya bisa manage user
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(
                      color: isSuper
                          ...
                          : (isAdminRole ? const Color(0xFF2563EB) : const Color(0xFFE2E8F0)),
                    ...
                    children: [
                      CircleAvatar(
                        ...
                        backgroundColor: isSuper
                            ...
                            : (isAdminRole ? const Color(0xFFEFF6FF) : const Color(0xFFF1F5F9)),
                        child: Icon(
                          isSuper ? Icons.shield_rounded : (isAdminRole ? Icons.admin_panel_settings_rounded : Icons.person_rounded),
                          color: isSuper ? const Color(0xFFB45309) : (isAdminRole ? const Color(0xFF2563EB) : const Color(0xFF64748B)),
                        ),
                      ),
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                              ...
                              children: [
                                ...
                                Container(
                                  ...
                                  decoration: BoxDecoration(
                                    color: isSuper
                                        ? const Color(0xFFFEF3C7)
                                        : (isAdminRole ? const Color(0xFFDBEAFE) : const Color(0xFFF1F5F9)),
                                    borderRadius: BorderRadius.circular(4),
                                  ),
                                  child: Text(
                                    isSuper ? 'SUPERADMIN' : user.role.name.toUpperCase(),
                                    style: TextStyle(
                                      fontSize: 9,
                                      fontWeight: FontWeight.bold,
                                      color: isSuper ? const Color(0xFFB45309) : (isAdminRole ? const Color(0xFF1E40AF) : const Color(0xFF475569)),
                                    ),
                                  ),
                                ),
                              ],
                            ...
                            Text(user.department, style: const TextStyle(fontSize: 11, color: Color(0xFF64748B))),
                          ],
                        ),
                      ),
                      if (canManage) ...[
                        ...
                        const Padding(
                          ...
                          child: Icon(Icons.lock_rounded, size: 18, color: Color(0xFFF59E0B)),
                        ),
                      ],
                    ],
                  ),
